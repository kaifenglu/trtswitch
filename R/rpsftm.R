...
rpsftm <- function(data, id = "id", stratum = "", time = "time", 
  ...
  if (length(id) == 0 || (length(id) == 1 && id[1] == "")) {
  ...
  if (length(treat) > 1) {
    stop("Only one treat variable can be specified.")
  }
  if (length(rx) > 1) {
    stop("Only one rx variable can be specified.")
  }
  if (length(censor_time) > 1) {
    stop("Only one censor_time variable can be specified.")
  }
  ...
  out <- rpsftmcpp(
    df = df, id = id, stratum = stratum, time = time, event = event,
