...
Rcpp::List ipcwcpp(const Rcpp::DataFrame df,
  ...
  auto f = [data, has_stratum, stratum, p_stratum, u_stratum, 
            ...
            n, p, p1, p2, q, covariates, numerator, denominator, 
  ...
  if (!boot) { // use Cox model to construct CI for HR if no boot
    ...
    struct BootstrapWorker : public RcppParallel::Worker {
      ...
      BootstrapWorker(int n_, int N_, int nids_, int ntss_,
        ...
        int c_lgs = z_lgs_denn.ncol;
        z_lgs_denc_local.resize(static_cast<std::size_t>(c_lgs));
      ...
      void operator()(std::size_t begin, std::size_t end) {
        ...
        for (std::size_t k = begin; k < end; ++k) {
          ...
          if (fails_out[k]) {
            ...
            append(boot_indexc_local, std::vector<int>(l, static_cast<int>(k + 1)));
    ...
    if (worker.index1_local > 0) {
      ...
      int c_lgs = worker.z_lgs_denc_local.size();
      for (int j = 0; j < c_lgs; ++j) {
        const std::string& zj = covariates_lgs_den[j];
        std::vector<double> u = std::move(worker.z_lgs_denc_local[j]);
