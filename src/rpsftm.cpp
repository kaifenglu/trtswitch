...
double est_psi_rpsftm(
  ...
  if (test == "logrank") {
    ...
    ListCpp fit = phregcpp(
      ...
      false, false, false, false, false, alpha);
    ...
    for (int j=0; j<q+p; ++j) {
      std::string zj = covariates_aft[j+1];
...
Rcpp::List rpsftmcpp(const Rcpp::DataFrame& df,
  ...
  if (has_stratum) {
    if (strata_main_effect_only) {
      ...
      for (int i = 0; i < p_stratum; ++i) {
        ...
        for (int j = 0; j < di; ++j) {
          for (int r = 0; r < n; ++r) {
            z_aftn(r, k + j) = stratan(r, i) == j ? 1.0 : 0.0;
          }
        }
      ...
      for (int j = 0; j < nstrata - 1; ++j) {
        ...
        for (int r = 0; r < n; ++r) {
          z_aftn(r, j) = stratumn[r] == j ? 1.0 : 0.0;
        }
      }
    }
  }
  ...
  event_summary.push_back(treat_out, "treated");
  event_summary.push_back(n_total, "n");
  event_summary.push_back(n_event, "event_n");
  event_summary.push_back(pct_event, "event_pct");
  event_summary.push_back(n_switch, "switch_n");
  event_summary.push_back(pct_switch, "switch_pct");
  ...
  double logRankPValue = lr.get<double>("logRankPValue")[0];
                ...
                const FlatMatrix& z_aftb) -> ListCpp {
                  ...
                  if (!psimissing) {
                    if (k == -1) {
                      ...
                      kmstar = kmestcpp(
                        Sstar, {"treated"}, "t_star", "",
                        "d_star", "", "log-log", 1-alpha, true);
                    }
                  ...
                  ListCpp out;
                  if (k == -1) {
                    out.push_back(eval_z, "eval_z");
                    out.push_back(Sstar, "Sstar");
                    out.push_back(kmstar, "kmstar");
                    out.push_back(data_outcome, "data_outcome");
                    out.push_back(km_outcome, "km_outcome");
                    out.push_back(lr_outcome, "lr_outcome");
                    out.push_back(fit_outcome, "fit_outcome");
                    out.push_back(psihat, "psihat");
                    out.push_back(psihat_vec, "psihat_vec");
  ...
  ListCpp out = f(idn, stratumn, timen, eventn, treatn, rxn,
                  censor_timen, zn, z_aftn);
  ...
  if (!psimissing) {
    ...
    if (!boot) { // use log-rank p-value to construct CI for HR if no boot
      ...
      int B = n * n_boot;
      std::vector<int> boot_indexc, idc, stratumc, treatc;
      std::vector<double> timec, eventc, rxc, censor_timec;
      boot_indexc.reserve(B); idc.reserve(B); stratumc.reserve(B); treatc.reserve(B);
      timec.reserve(B); eventc.reserve(B); rxc.reserve(B); censor_timec.reserve(B);
      FlatMatrix z_aftc(B, q + p);
      ...
      for (int k = 0; k < n_boot; ++k) {
        ...
        for (int h = 0; h < ntss; ++h) {
          int start = tsx[h], end = tsx[h+1], block_len = end - start;
          std::vector<int> indices(block_len);
          for (int i = start; i < end; ++i) {
            double u = dist(rng);
            int j = tsx[h] + static_cast<int>(std::floor(u * block_len));
            indices[i - start] = j;
            idb[i] = idn[j];
          ...
          for (int l = 0; l < z_aftb.ncol; ++l) {
            for (int i = start; i < end; ++i) {
              int j = indices[i - start];
              z_aftb(i, l) = z_aftn(j, l);
            }
          }
        }

        ListCpp out = f(idb, stratumb, timeb, eventb, treatb, rxb,
                        censor_timeb, zb, z_aftb);

        fails[k] = out.get<bool>("fail");
        hrhats[k] = out.get<double>("hrhat");
        psihats[k] = out.get<double>("psihat");

        if (fails[k]) {
          for (int i = 0; i < n; ++i) {
            boot_indexc.push_back(k + 1);
            idc.push_back(idb[i]);
            stratumc.push_back(stratumb[i]);
            timec.push_back(timeb[i]);
            eventc.push_back(eventb[i]);
            treatc.push_back(treatb[i]);
            rxc.push_back(rxb[i]);
            censor_timec.push_back(censor_timeb[i]);
          }
          for (int l = 0; l < z_aftb.ncol; ++l) {
            for (int i = 0; i < n; ++i) {
              int j = index1 + i;
              z_aftc(j, l) = z_aftb(i, l);
            }
          }
          index1 += n;
        }
      ...
      if (!boot_indexc.empty()) {
        std::vector<int> sub = seqcpp(0, index1 - 1);
        subset_in_place_flatmatrix(z_aftc, sub);
        ...
        fail_boots_data.push_back(std::move(treatc), "treated");
      ...
      std::transform(hrhats1.begin(), hrhats1.end(), loghrs.begin(),
                     [](int value) { return std::log(value); });
  ...
  result.push_back(psihat_vec, "psi_roots");
  result.push_back(psi_CI, "psi_CI");
  result.push_back(psi_CI_type, "psi_CI_type");
  result.push_back(logRankPValue, "logrank_pvalue");
  result.push_back(pvalue, "cox_pvalue");
  result.push_back(hrhat, "hr");
  result.push_back(hr_CI, "hr_CI");
  result.push_back(hr_CI_type, "hr_CI_type");
  result.push_back(event_summary, "event_summary");
  result.push_back(eval_z, "eval_z");
  result.push_back(Sstar, "Sstar");
  result.push_back(kmstar, "kmstar");
  result.push_back(data_outcome, "data_outcome");
  result.push_back(km_outcome, "km_outcome");
  result.push_back(lr_outcome, "lr_outcome");
  result.push_back(fit_outcome, "fit_outcome");
