...
static FlatMatrix make_nan_matrix(size_t rows, size_t cols) {
...
FlatMatrix dersBasisFuns(size_t i, double u, size_t p, size_t n, 
  ...
  for (size_t r = 0; r <= p; ++r) {
    ...
    for (size_t k = 1; k <= n; ++k) {
      ...
      size_t j1 = (r >= k) ? 1 : (k - r);
      size_t j2 = std::min(k-1, p-r);
      for (size_t j = j1; j <= j2; ++j) {
        a(s2,j) = (a(s1,j) - a(s1,j-1)) / ndu(pk+1, j + r - k);
        d += a(s2,j) * ndu(j + r - k, pk);
      }
...
FlatMatrix splineDesigncpp(
    ...
    size_t ord,
    const std::vector<size_t>& derivs)
{
  ...
  for (size_t i = 0; i < nx; ++i) {
    ...
    size_t deriv_order = derivs2[i];
    if (deriv_order > p) deriv_order = p;
    
    // find span
    size_t span = findSpan(n, p, xv, knots);
    ...
    for (size_t j = 0; j <= p; ++j) {
      if (j + span >= p && j + span < p + ncol) {
        size_t global_i = j + span - p;
...
ListCpp bscpp(
    const std::vector<double>& x,
    size_t df, // number of columns of the basis matrix
    const std::vector<double>& knots_in,
    size_t degree,
    bool intercept,
    ...
    bool warn_outside)
{
  ...
  // NA handling
  std::vector<size_t> non_na_idx = indices_not_nan(x);
  ...
  for (size_t k = 0; k < ord; ++k) u[k] = boundary_knots[0];
  for (size_t k = ord; k < K + ord; ++k) u[k] = knots[k-ord];
  for (size_t k = K + ord; k < K + 2 * ord; ++k) u[k] = boundary_knots[1];
  
  // Prepare deriv0 vector
  std::vector<size_t> deriv0(1, 0);
  ...
  if (std::any_of(outside.begin(), outside.end(), [](char v){ return v; })) {
    ...
    scalef[0] = 1.0;
    for (size_t i = 1; i < ord; ++i) scalef[i] = scalef[i-1] * i;
    ...
    if (std::any_of(outleft.begin(), outleft.end(), [](char v){ return v; })) {
      ...
      std::vector<size_t> idx = which(outleft);
      std::vector<double> zol = subset(z, idx);
      size_t r = zol.size();
      
      FlatMatrix zl(r, ord);
      double* zptr = zl.data_ptr();
      for (size_t i = 0; i < r; ++i) zptr[i] = 1.0;
      for (size_t j = 1; j <= degree; ++j) {
        double* zcol = zptr + j * r;
        for (size_t i = 0; i < r; ++i) zcol[i] = std::pow(zol[i] - k_pivot, j);
      }
      
      std::vector<double> kp(ord, k_pivot);
      std::vector<size_t> derivs(ord);
      for (size_t i = 0; i < ord; ++i) derivs[i] = i;
      FlatMatrix tt = splineDesigncpp(u, kp, ord, derivs);
      
      for (size_t j = 0; j < Lfull; ++j) {
        double* design_col = design.data_ptr() + j * n;
        for (size_t l = 0; l < ord; ++l) {
    ...
    if (std::any_of(outright.begin(), outright.end(), [](char v){ return v; })) {
      ...
      std::vector<size_t> idx = which(outright);
      ...
      for (size_t j = 1; j <= degree; ++j) {
        double* zcol = zptr + j * r;
        for (size_t i = 0; i < r; ++i) zcol[i] = std::pow(zor[i] - k_pivot, j);
      }
    ...
    for (size_t i = 0; i < n; ++i) inside[i] = !outside[i];
    if (std::any_of(inside.begin(), inside.end(), [](char v){ return v; })) {
      std::vector<size_t> idx = which(inside);
  ...
  size_t ncol_full = Lfull;
  size_t col_from = (intercept ? 0 : 1);
  size_t ncol = ncol_full - col_from;
  if (ncol <= 0) ncol = 0;
    ...
    const std::vector<double>& boundary_knots_in)
{
  ...
  for (size_t k = 0; k < 4; ++k) u[k] = boundary_knots[0];
  for (size_t k = 4; k < K + 4; ++k) u[k] = knots[k - 4];
  for (size_t k = K + 4; k < K + 8; ++k) u[k] = boundary_knots[1];
  ...
  if (std::any_of(outside.begin(), outside.end(), [](char v){ return v; })) {
    ...
    if (std::any_of(outleft.begin(), outleft.end(), [](char v){ return v; })) {
      ...
      FlatMatrix zl(r,2);
      for (size_t i = 0; i < r; ++i) zl(i,0) = 1.0;
      for (size_t i = 0; i < r; ++i) zl(i,1) = zol[i] - k_pivot;
      
      std::vector<double> kp(2, k_pivot);
      std::vector<size_t> derivs = {0, 1};
    ...
    if (std::any_of(outright.begin(), outright.end(), [](char v){ return v; })) {
      ...
      FlatMatrix zr(r,2);
      for (size_t i = 0; i < r; ++i) zr(i,0) = 1.0;
      for (size_t i = 0; i < r; ++i) zr(i,1) = zor[i] - k_pivot;
