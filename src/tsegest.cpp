  ...
  if (!psimissing) {
    ...
    if (!boot) { // use Cox model to construct CI for HR if no boot
      ...
      if (is_true(any(fails))) {
        ...
        zc_lgs = subset_matrix_by_row(zc_lgs,sub);
